...
if ($_POST["useremail"] === "" && $_POST["userpass"] ==="") {
  ...
  if(empty($_POST["useremail"]) || empty($_POST["userpass"])) {
    ...
    if (!password_verify($_POST['userpass'], $resultLogin['password'])) {
      ...
      header("Location: userPortal_login.php"); //ログイン後のページにリダイレクト
      exit();
    }
  }
}
...
<div class="title">
    ...
    <form class="userentry" action="userLogin.php" method="post">
<div class="mb-3">
...
<input type="submit" class="btn btn-info btn-sm h-50" role="button" style="margin-top: 20px;" value="login"></input>
</form>
</section>
